...
class Cloud {
	...
	public firstRadius: number;
	public secondRadius: number;
	public thirdRadius: number;
}

var canvas = document.createElement("canvas");
var ctx = canvas.getContext("2d");

var drawThinLine = function (start: Vector, end: Vector) {
	ctx.moveTo(Math.floor(start.x) + 0.5, Math.floor(start.y) + 0.5);
	ctx.lineTo(Math.floor(end.x) + 0.5, Math.floor(end.y) + 0.5);
};

var current = null;
var starPositions: Vector[] = [];
var clouds: Cloud[][] = [];
...
var doAnimation = function (timestamp) {
	...
	for (var i = 0; i < starPositions.length * 0.2; i++) {
		drawThinLine(
			new Vector(starPositions[i].x, starPositions[i].y - 2),
			new Vector(starPositions[i].x, starPositions[i].y + 2)
			);
		drawThinLine(
			new Vector(starPositions[i].x - 2, starPositions[i].y),
			new Vector(starPositions[i].x + 2, starPositions[i].y)
			);
	}
	ctx.closePath();
	...
	for (var i = Math.floor(starPositions.length * 0.2); i < starPositions.length * 0.5; i++) {
		drawThinLine(
			new Vector(starPositions[i].x, starPositions[i].y - 1),
			new Vector(starPositions[i].x, starPositions[i].y + 1)
			);
		drawThinLine(
			new Vector(starPositions[i].x - 1, starPositions[i].y),
			new Vector(starPositions[i].x + 1, starPositions[i].y)
			);
	}
	ctx.closePath();
	...
	ctx.beginPath();
	ctx.moveTo(42.5, height - 300); //Body
	ctx.lineTo(42.5, height - 300 + 30);

	ctx.moveTo(42.5, height - 300 + 5); //Right hand
	ctx.lineTo(47.5, height - 300 - 15);

	ctx.moveTo(42.5, height - 300 + 5); //Left hand
	ctx.lineTo(40, height - 300 + 25);

	ctx.moveTo(42.5, height - 300 + 30); //Left leg
	ctx.lineTo(44.5, height - 300 + 40);
	ctx.moveTo(44.5, height - 300 + 40);
	ctx.lineTo(42.5, height - 300 + 50);

	ctx.moveTo(42.5, height - 300 + 30); //Right leg
	...
	ctx.strokeStyle = "white";
	ctx.beginPath();
	ctx.moveTo(47.5, height - 300 - 15); //Rope
	...
	ctx.beginPath();
	ctx.arc(46.5, height - 300 - 35, 1.5, 0, 2 * Math.PI); //Firefly
	ctx.closePath();
	ctx.fill();
	...
	ctx.fillStyle = glow;
	ctx.beginPath();
	ctx.arc(46.5, height - 300 - 35, 30, 0, 2 * Math.PI); //Firefly glow
